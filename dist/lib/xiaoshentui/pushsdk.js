...
var _typeof3 = require('./../../npm/babel-runtime/helpers/typeof.js');
...
exports.pushSdk = function () {
    ...
    wx.getSetting({
      success: function success(res) {
        if (res.authSetting['scope.userInfo']) {
          wx.getUserInfo({
            success: function success(res) {
              InitData.userInfo = res.userInfo;
              push_log(InitData, 'setopenid', 'setopenid');
            }
          });
        } else {
          push_log(InitData, 'setopenid', 'setopenid');
  ...
  var pubicFunc = {
    setOpenId: function setOpenId(openid) {
      if (openid.length !== 28) {
        console.error('openId位数不对');
        return;
      }
      InitData.openid = openid;
      if (typeof openid !== 'string' || openid.length !== 28) {
        console.error('openid格式或位数错误');
        return;
      }
      sentOpenId();
    },
    sentEvent: function sentEvent(obj) {
      if (obj.eventId.length != 32) {
        console.error('eventId位数不对');
        return;
      }
      if (obj.arg === undefined || obj.arg === '') {
        delete obj.arg;
      } else {
        if (checkType(obj.arg) !== 'Object') {
          console.error('arg属性应为空字符串或对象');
          return;
        }
      }
      for (var key in obj.arg) {
        if (_typeof(obj.arg[key]) === 'object') {
          console.error('value只能是字符串');
          return;
        }
      }
      InitData.eventId = obj.eventId;
      InitData.arg = obj.arg;
      push_log(InitData, 'tagEvent');
    },
    sendJsCode: function sendJsCode(jscode) {
      if (typeof jscode === 'string') {
        InitData.jscode = jscode;
      }
      wx.getSetting({
        success: function success(res) {
          if (res.authSetting['scope.userInfo']) {
  ...
  function HookIt1(obj, method, callback) {
    if (obj[method]) {
  ...
  function get_uuid() {
    var uuid = wx.getStorageSync('t_uuid');
    if (!uuid) {
      uuid = '' + Date.now() + Math.floor(Math.random() * 1e7);
      wx.setStorageSync('t_uuid', uuid);
  ...
  var wx_request = function wx_request(data, method, uri) {
    ...
    if ((0, _stringify2.default)(data).length >= 4e3) {
      return;
    }
    var retryTimes = 0;
    var sendLog = function sendLog() {
      wx.request({
        url: 'https://' + url + '.xiaoshentui.com/' + uri,
        data: data,
        header: {},
        method: method,
  ...
  function push_log(InitData, ev, life) {
    var uuid = get_uuid();
    var copy_ifo = void 0;
    if (ev == 'app' && life == 'hide') {
      copy_ifo = wx.getStorageSync('ifo');
    ...
    var data = {
      ...
      st: InitData.st || 0,
      et: ev == 'app' && life == 'hide' ? Date.now() : 0,
      ppx: InitData.ppx ? InitData.ppx : 0,
      ppy: InitData.ppy ? InitData.ppy : 0,
      data: ev == 'user_info' ? InitData.userInfo : 0,
      fid: ev == 'fpage' ? InitData.fid : 0,
      lang: InitData.lang ? InitData.lang : 0,
      wsr: ev == 'app' ? InitData.appOptions : {},
      ifo: copy_ifo || 0,
      jscode: ev !== 'user_info' ? 0 : InitData.jscode,
      ust: Date.now(),
      openid: InitData.openid ? InitData.openid : 0,
  ...
  function weaOnlinePushLayer() {
    var resSystem = wx.getSystemInfoSync();
    var data = {
      app_key: app_key,
      uuid: wx.getStorageSync('t_uuid'),
      openid: InitData.openid ? InitData.openid : '',
      os: resSystem.model,
      device: resSystem.brand
    };
    wx.request({
  ...
  function atDetails(e) {
    var _this = this;
    var elData = e.currentTarget.dataset;
    var isShow = onlineData;
    var resSystem = wx.getSystemInfoSync();
    var data = {
      app_key: app_key,
      uuid: wx.getStorageSync('t_uuid'),
      os: resSystem.model,
      device: resSystem.brand,
      msg_key: elData['msgkey'] ? elData['msgkey'] : ''
    };
    if (elData['msgkey']) {
      wx.request({
    ...
    } else if (elData.type == 3) {
      wx.navigateToMiniProgram({
        appId: elData['apd'],
        path: '/' + elData['acdetail'],
        success: function success(res) {
          isShow['isShow'] = false;
          _this.setData({
            onlineData: isShow
  ...
  return function (miniProLife, at) {
    ...
    function pushAppOnLaunch(options) {
      get_uuid();
      if (typeof options !== 'undefined') {
        InitData.appOptions = options;
        InitData.pp = options['path'];
      } else {
        InitData.appOptions = {};
      }
      if (app_key) {
        if (!wx.getStorageSync('t_appkey')) {
          wx.setStorageSync('t_appkey', app_key);
        }
      }
    }

    function pushAppOnShow(options) {
      if (typeof options !== 'undefined') {
        InitData.appOptions = options;
      } else {
        InitData.appOptions = {};
    ...
    function pushAppOnHide() {
      sentOpenId();
      push_log(InitData, 'app', 'hide');
    }

    function pushPageOnShow(options) {
      var _t = this;
      InitData.st = Date.now();
      InitData.pp = this['__route__'];
      if (_t.onShareAppMessage) {
        var oldShare = _t.onShareAppMessage;
        var element;
        _t.onShareAppMessage = function (res) {
          if (res.from === 'button') {
            element = 'button';
          } else {
            element = 'menu';
          }
          InitData.ele = element;
          push_log(InitData, 'share');
          return oldShare(res);
        };
      }
      setTimeout(function () {
        if (onlineData['isShow'] && onlineTier) {
          ...
          if (!_t.colseOneBox) {
            HookIt1(_t, 'atDetails', atDetails);
            HookIt1(_t, 'colseOneBox', colseOneBox);
          }
        }
      }, 2e3);
    }

    function pushPageOnHide(options) {
      var app = this;
      if (typeof options !== 'undefined') {
        app.options = options;
      }
      InitData.pp = app['__route__'];
      if (onlineData['isShow']) {
        onlineTier = false;
        onlineData = {};
        this.setData({
          onlineTier: onlineTier,
    ...
    var pushApp = function pushApp(arg) {
      ...
      HookIt1(arg, 'onHide', pushAppOnHide);
      ohterApp ? ohterApp(arg) : App(arg);
    };
    var pushPage = function pushPage(arg) {
      HookIt1(arg, 'onShow', pushPageOnShow);
      HookIt1(arg, 'onHide', pushPageOnHide);
